...
//
//   - **`lang: "00"` means textless, not a language.** It maps to an empty
//     Language, which is what lets the Platform prefer a backdrop with no burned-in
//     title to sit under a hero's clearlogo. Carried through as a language, every
//     textless image would look foreign and the preference would never fire.
//   - **An HD variant outranks its standard-definition twin.** Likes accumulate
//     per image, so an older SD logo frequently has more of them than the HD one
...
//   - **The artwork type table is a guess that must be verified.** The mapping
//     from fanart.tv's response keys to slots is data in one table precisely so
//     that a type which has been renamed or added is a one-line correction. Check
//     it against fanart.tv's current documentation before trusting the coverage.
package fanarttv
